...
const I18N = {
  he: {
    ...
    'tabs.destinations': 'יעדי משלוח',
    'pod.deliveryNote': '🚚 תעודת משלוח',
    'pod.destination': 'יעד משלוח',
    'dest.errNeedMigration': 'טבלת היעדים אינה קיימת עדיין — יש להריץ את מיגרציה 20260824000000 ב-SQL Editor',
    'dest.empty': 'עדיין אין יעדי משלוח בפרויקט. ייבאו את רשימת החדרים מהמזמין.',
    'dest.groundFloor': 'קומת קרקע',
    'dest.floorN': 'קומה {n}',
    'dest.buildingN': 'בניין {b}',
    'dest.allBuildings': 'כל הבניינים',
    'dest.addressLine': 'בניין {building} · {floor} · חדר {room}',
    'dest.room': 'חדר',
    'dest.expected': 'טיפוס מתוכנן',
    'dest.assignedPod': 'פוד משויך',
    'dest.assignedPanel': 'פאנל משויך',
    'dest.noPod': 'לא שויך',
    'dest.assign': 'שייך',
    'dest.change': 'החלף',
    'dest.unassign': 'נתק',
    'dest.assignTitle': 'שיוך יחידה — {address}',
    'dest.searchPod': 'חיפוש לפי קוד יחידה…',
    'dest.matches': 'מתאים',
    'dest.noFreePods': 'אין יחידות פנויות לשיוך',
    'dest.assigned': 'היחידה שויכה ליעד',
    'dest.errAssign': 'שגיאה בשיוך היחידה',
    'dest.errRoomTaken': 'לחדר הזה כבר משויכת יחידה אחרת',
    'dest.statTotal': 'סה"כ יעדים',
    'dest.statAssigned': 'משויכים',
    'dest.statUnassigned': 'חדרים ללא יחידה',
    'dest.statPodsNoDest': 'פודים ללא יעד',
    'dest.statPanelsNoDest': 'פאנלים ללא יעד',
    'dest.autoAssign': '⚡ שיוך אוטומטי',
    'dest.autoConfirm': 'לשייך {n} יחידות מתוך {rooms} חדרים פתוחים? ({noType} חדרים ללא טיפוס מתוכנן יידלגו וישויכו ידנית)',
    'dest.autoNothing': 'לא נמצאה אף יחידה פנויה שמתאימה לחדרים הפתוחים',
    'dest.allAssigned': 'כל החדרים כבר משויכים',
    'dest.autoDone': '{n} יחידות שויכו',
    'dest.autoDonePartial': '{n} יחידות שויכו, {failed} נכשלו',
    'dest.confirmDelete': 'למחוק את היעד {address}? היחידה המשויכת לא תימחק — רק תאבד את הכתובת.',
    'dest.errDelete': 'שגיאה במחיקת היעד',
    'dest.deleted': 'היעד נמחק',
    'dest.importAction': '📥 ייבוא יעדים',
    'dest.importTitle': 'ייבוא רשימת יעדים',
    'dest.importHelp': 'עמודות לפי הסדר: בניין | קומה (0 = קרקע) | קוד חדר | טיפוס (אופציונלי) | כיוון R/L (אופציונלי). אפשר להעלות קובץ אקסל או להדביק ישירות מ-Excel. שורת כותרת מזוהה ומדולגת.',
    'dest.importFile': 'קובץ אקסל / CSV',
    'dest.importPaste': 'או הדבקה מ-Excel',
    'dest.previewNew': '{n} יעדים חדשים ייובאו',
    'dest.previewExisting': '{n} שורות כבר קיימות ויידלגו',
    'dest.previewBad': '{n} שורות לא תקינות (יידלגו), שורות',
    'dest.nothingToImport': 'אין שורות חדשות לייבוא',
    'dest.imported': '{n} יעדים יובאו',
    'dest.errImport': 'שגיאה בייבוא: ',
    'dest.errFileRead': 'לא ניתן לקרוא את הקובץ',
    'dest.exportExcel': '📊 ייצוא לאקסל',
    'dest.sheetName': 'יעדי משלוח',
    'dest.hBuilding': 'בניין',
    'dest.hFloor': 'קומה',
    'dest.hRoom': 'חדר',
    'dest.hExpected': 'טיפוס מתוכנן',
    'dest.noteTitle': 'תעודת משלוח',
    'dest.noteDate': 'תאריך:',
    'dest.noteAddress': 'כתובת יעד באתר',
    'dest.noteNoAddress': 'לא הוגדרה כתובת יעד',
    'dest.noteReceived': 'הריני לאשר כי היחידה שלעיל התקבלה במצב תקין ובמיקום המצוין בתעודה זו.',
    'dest.noteSenderName': 'שם המוסר',
    'dest.noteReceiverName': 'שם המקבל',
    'dest.noteSenderSign': 'חתימת המוסר',
    'dest.noteReceiverSign': 'חתימת המקבל',
    'dest.noteRemarks': 'הערות',
    ...
    'filter.stageOptShort': 'שלב {letter}',
    ...
    'proj.targetSummaryTitle': '🎯 יעד ייצור לפרויקט',
    'proj.targetQty': 'כמות יעד',
    'proj.targetQtyRight': 'יעד ימין',
    'proj.targetQtyLeft': 'יעד שמאל',
    'proj.targetQtyHint': 'כמות היעד הסופית לכל טיפוס וכיוון. יחידות שהוזנו למערכת מופחתות ממנה בטבלת "יעד ייצור" בטאב הפודים; שדה ריק = ללא יעד.',
    'proj.targetNeedMigration': 'להגדרת כמויות יעד יש להריץ את מיגרציה 20260824010000 ב-SQL Editor',
    'proj.targetEntered': 'הוזנו',
    'proj.targetScrapped': 'נגרעו',
    'proj.targetRemaining': 'נשארו לבצע',
    'proj.targetTotal': 'סה"כ',
    'proj.targetOverTitle': 'הוזנו יותר יחידות מכמות היעד',
    'proj.targetCollapse': 'צמצם / הרחב',
    'proj.targetCollapsedSummary': 'נשארו לבצע {remaining} מתוך {target}',
    'pod.scrapped': 'נגרע',
    'pod.scrappedBadge': '✖ נגרע',
    'pod.scrappedTitle': 'פוד נגרע — אינו נספר בכמות היעד',
    'pod.scrapAction': '✖ סמן כנגרע',
    'pod.unscrapAction': '↩ בטל גריעה',
    'pod.scrapTitle': 'גריעת יחידה {code}',
    'pod.scrapExplain': 'היחידה תסומן כנגרעת: היא לא תיספר בכמות היעד של הפרויקט ולא תוצע לשיוך יעד משלוח. היא אינה נמחקת — היסטוריית הבדיקות נשמרת והפעולה מתועדת ביומן השינויים.',
    'pod.scrapReason': 'סיבת הגריעה',
    'pod.scrapReasonPh': 'למשל: סדק ברצפת הבטון לאחר יציקה',
    'pod.scrapNeedReason': 'חובה לציין סיבת גריעה',
    'pod.scrapConfirm': 'גרע יחידה',
    'pod.scrapDone': 'היחידה נגרעה מכמות היעד',
    'pod.unscrapConfirm': 'לבטל את הגריעה של {code}? היחידה תחזור להיספר בכמות היעד.',
    'pod.unscrapDone': 'הגריעה בוטלה',
    'pod.scrapError': 'שגיאה בעדכון הגריעה: ',
    'audit.pod_scrapped': 'היחידה נגרעה',
    'audit.pod_unscrapped': 'הגריעה בוטלה',
    ...
    // הוספת טיפוס לפרויקט קיים — אין מיגרציה, אותם שני INSERT של createProject
    'proj.addType': 'הוסף טיפוס',
    'proj.addModelType': 'הוסף דגם',
    'proj.addTypeTitle': 'הוספת טיפוס לפרויקט',
    'proj.addModelTitle': 'הוספת דגם לפרויקט',
    'proj.addTypeNumberHint': 'ייווצר כ-T{n}, בהמשך למספור הקיים בפרויקט.',
    'proj.addTypeDirs': 'כיוונים וכמות יעד',
    'proj.addTypeNoDirs': 'יש לבחור לפחות כיוון אחד',
    'proj.addTypePodsHint': 'הוספת טיפוס אינה יוצרת יחידות. לאחר השמירה הטיפוס יופיע בהרכב של קבוצת ביצוע חדשה ובמודל "הוסף פוד" — שם המספור הסידורי ממשיך ברצף של הפרויקט.',
    'proj.typeCreated': '{label} נוצר בהצלחה',
    'proj.length': 'אורך',
    'proj.width': 'רוחב',
    'proj.height': 'גובה',
  ...
  en: {
    ...
    'pod.deliveryNote': '🚚 Delivery note',
    'pod.destination': 'Destination',
    'dest.errNeedMigration': 'The destinations table does not exist yet — run migration 20260824000000 in the SQL Editor',
    'dest.empty': 'No delivery destinations yet. Import the room list supplied by the client.',
    'dest.groundFloor': 'Ground floor',
    'dest.floorN': 'Floor {n}',
    'dest.buildingN': 'Building {b}',
    'dest.allBuildings': 'All buildings',
    'dest.addressLine': 'Building {building} · {floor} · Room {room}',
    'dest.room': 'Room',
    'dest.expected': 'Planned type',
    'dest.assignedPod': 'Assigned pod',
    'dest.assignedPanel': 'Assigned panel',
    'dest.noPod': 'Unassigned',
    'dest.assign': 'Assign',
    'dest.change': 'Change',
    'dest.unassign': 'Detach',
    'dest.assignTitle': 'Assign a unit — {address}',
    'dest.searchPod': 'Search by unit code…',
    'dest.matches': 'match',
    'dest.noFreePods': 'No unassigned units available',
    'dest.assigned': 'Unit assigned to the destination',
    'dest.errAssign': 'Could not assign the unit',
    'dest.errRoomTaken': 'Another unit is already assigned to this room',
    'dest.statTotal': 'Destinations',
    'dest.statAssigned': 'Assigned',
    'dest.statUnassigned': 'Rooms without a unit',
    'dest.statPodsNoDest': 'Pods without a destination',
    'dest.statPanelsNoDest': 'Panels without a destination',
    'dest.autoAssign': '⚡ Auto-assign',
    'dest.autoConfirm': 'Assign {n} units across {rooms} open rooms? ({noType} rooms have no planned type and are left for manual assignment)',
    'dest.autoNothing': 'No available unit matches the open rooms',
    'dest.allAssigned': 'Every room is already assigned',
    'dest.autoDone': '{n} units assigned',
    'dest.autoDonePartial': '{n} units assigned, {failed} failed',
    'dest.confirmDelete': 'Delete destination {address}? The assigned unit is kept — it just loses its address.',
    'dest.errDelete': 'Could not delete the destination',
    'dest.deleted': 'Destination deleted',
    'dest.importAction': '📥 Import destinations',
    'dest.importTitle': 'Import destination list',
    'dest.importHelp': 'Columns in order: building | floor (0 = ground) | room code | type (optional) | direction R/L (optional). Upload a spreadsheet or paste straight from Excel. A header row is detected and skipped.',
    'dest.importFile': 'Excel / CSV file',
    'dest.importPaste': 'Or paste from Excel',
    'dest.previewNew': '{n} new destinations will be imported',
    'dest.previewExisting': '{n} rows already exist and will be skipped',
    'dest.previewBad': '{n} invalid rows (skipped), lines',
    'dest.nothingToImport': 'No new rows to import',
    'dest.imported': '{n} destinations imported',
    'dest.errImport': 'Import failed: ',
    'dest.errFileRead': 'The file could not be read',
    'dest.exportExcel': '📊 Export to Excel',
    'dest.sheetName': 'Destinations',
    'dest.hBuilding': 'Building',
    'dest.hFloor': 'Floor',
    'dest.hRoom': 'Room',
    'dest.hExpected': 'Planned type',
    'dest.noteTitle': 'Delivery note',
    'dest.noteDate': 'Date:',
    'dest.noteAddress': 'Destination address on site',
    'dest.noteNoAddress': 'No destination address set',
    'dest.noteReceived': 'I confirm the unit above was received in good condition at the location stated in this note.',
    'dest.noteSenderName': 'Sender name',
    'dest.noteReceiverName': 'Receiver name',
    'dest.noteSenderSign': 'Sender signature',
    'dest.noteReceiverSign': 'Receiver signature',
    'dest.noteRemarks': 'Remarks',
    ...
    'filter.stageOptShort': 'Stage {letter}',
    ...
    'proj.targetSummaryTitle': '🎯 Production target',
    'proj.targetQty': 'Target qty',
    'proj.targetQtyRight': 'Target R',
    'proj.targetQtyLeft': 'Target L',
    'proj.targetQtyHint': 'The project\'s final quantity per type and direction. Units entered into the system are subtracted in the "Production target" table on the pods tab; empty = no target.',
    'proj.targetNeedMigration': 'To set target quantities, run migration 20260824010000 in the SQL Editor',
    'proj.targetEntered': 'Entered',
    'proj.targetScrapped': 'Scrapped',
    'proj.targetRemaining': 'Remaining',
    'proj.targetTotal': 'Total',
    'proj.targetOverTitle': 'More units entered than the target quantity',
    'proj.targetCollapse': 'Collapse / expand',
    'proj.targetCollapsedSummary': '{remaining} of {target} remaining',
    'pod.scrapped': 'Scrapped',
    'pod.scrappedBadge': '✖ Scrapped',
    'pod.scrappedTitle': 'Scrapped unit — not counted toward the target',
    'pod.scrapAction': '✖ Mark as scrapped',
    'pod.unscrapAction': '↩ Undo scrap',
    'pod.scrapTitle': 'Scrap unit {code}',
    'pod.scrapExplain': 'The unit will be marked scrapped: it stops counting toward the project target and is no longer offered a delivery destination. It is not deleted — its inspection history is kept and the action is written to the audit log.',
    'pod.scrapReason': 'Scrap reason',
    'pod.scrapReasonPh': 'e.g. crack in the concrete floor after casting',
    'pod.scrapNeedReason': 'A scrap reason is required',
    'pod.scrapConfirm': 'Scrap unit',
    'pod.scrapDone': 'The unit was marked scrapped',
    'pod.unscrapConfirm': 'Undo the scrap of {code}? The unit will count toward the target again.',
    'pod.unscrapDone': 'Scrap undone',
    'pod.scrapError': 'Could not update the scrap flag: ',
    'audit.pod_scrapped': 'Unit scrapped',
    'audit.pod_unscrapped': 'Scrap undone',
    'pod.history': '📜 History',
    ...
    'proj.addType': 'Add type',
    'proj.addModelType': 'Add model',
    'proj.addTypeTitle': 'Add a type to the project',
    'proj.addModelTitle': 'Add a model to the project',
    'proj.addTypeNumberHint': 'Will be created as T{n}, continuing the project\'s existing numbering.',
    'proj.addTypeDirs': 'Directions and target quantity',
    'proj.addTypeNoDirs': 'Select at least one direction',
    'proj.addTypePodsHint': 'Adding a type does not create units. Once saved, the type appears in a new production group\'s composition and in the "add pod" dialog — where serial numbering continues the project\'s sequence.',
    'proj.typeCreated': '{label} created successfully',
    'proj.length': 'Length',
    'proj.width': 'Width',
    'proj.height': 'Height',
